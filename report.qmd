...
This was then visualized for always the first column. Here we only show the plots for the 0.025 and 0.05 thresholds on transcript and gene level because all 8 histograms  look almost the same. The rest can be seen in the [file](code/qmd/comparison_pacbio_illumina.qmd) file. 
...
So first, the correlation not stratified by length was plotted. We performed both pearson- and spearman-correlation but only the spearman plot of the first NDR threshold is shown for transcript and gene level (explained in discussion).
...
Then, the correlation stratified by length was plotted for all 4 NDR thresholds. For the purpose of no further information gain we will only show one pearson plot and then representatively one spearman heat map (for one bin) for each NDR threshold (we chose 1000-2000 as its the most abundant transcript length) as well as all 4 scatter plots of the summed up correlation stratified by length. The individual heat maps are not shown in the report but can be commented out in the .qmd file for their visualization.   
...
The pearson plots do not look like usual heat maps as the day 2 - rep 1 - Bambu is highly correlated with all samples of Salmon and the rest of the correlations seem quite low. When observing the 0.025 heat map with spearman correlation one sees most of the samples being highly correlated with themselves (or the replicates) and a bit clearer the highly correlated diagonal usually expected in heat maps. 
...
```
The MDS plots look very similar between all the 4 NDR thresholds. When comparing the MDS plots between Salmon and Bambu the MDS1 separation by day is almost the same with the MDS2 separation being a bit more different. In Bambu the replicated are grouped a bit closer together than in Salmon and fewer samples are above 0. 
...
Since the DGE analysis of the different NDR thresholds showed very similar results, only the 0.2 NDR threshold is shown here. The analysis of all 4 NDR thresholds can be found in the files [file](code/html/dge_analysis.html) and [file](code/qmd/dge_analysis.qmd).

#### Load data

The data is loaded from RDS-objects that have been created with the [file](code/qmd/import_processing.qmd) file. For each of the 4 NDR thresholds (0.025, 0.05, 0.1, 0.2), two data frames containing the expression data (counts) of the two different technologies exist:
...
For detailed explanations of the steps refer to the [file](code/qmd/DTU_test_analysis.qmd).
...
After DRIMSeq filtering, 10364 (`r round(10364/62938*100, 2)`%) of genes and  28447 (`r round(28447/249108*100, 2)`) \% of features remained in the Illumina+Salmon data for further analysis. For PacBio+Bambu 7847 (`r round(7847/62938*100, 2)`%) genes and (`r round(19816/249108*100, 2)`) features passed the filter.
...
The results for all 4 thresholds were similar so some plots were omitted in the report and can be viewed [here](`code/qmd/comparison_pacbio_illumina_tpm.qmd`). The small differences can be explained by the fact that the NDR thresholds determine the number of newly detected genes which doesn't have an effect on the comparison of both methods.
Secondly, the analysis was performed on count and TPM data. It was decided to keep in only the result for TPM due to stratification by transcript length making these results more interesting, and also because the further analysis steps were performed on TPM rather than the raw count data. For completeness the count results can be found in a separate [file](code/qmd/comparison_pacbio_illumina_counts.qmd).

#### Differences 

Further, the differences between the Illumina data (short read data produced by a thoroughly optimized and long-in-use method) and the long-read PacBio data (has not been optimized well with biases) were investigated. In the raw data, the differences seemed to be quite small (the majority being <50) as well as the highest bar of the histograms being around 0. This was shown here only for the first column as all the other columns look very similar. This would suggest that the Illumina and PacBio raw data seem to be quite similar with only minor deviations. 

#### Correlation and stratification by length 

Next, the correlation between Illumina and PacBio data was investigated. Both, pearson and spearman correlation, were performed which differed highly. For pearson correlation, one can't say that each sample is highest correlated with itself as some samples are highly correlated with all samples of all days (usually sample day 1 or 2) whilst the spearman heat maps looks as expected. This can be explained by the fact that spearman correlation is more robust to outliers than pearson and doesn't assume a linear relationship which works better with our data. 
Also, the data was stratified into intervals of transcript length 1000 which resulted in the bin 1000-2000 containing the most transcripts and making them an irregular size. But this evened out as both Bambu and Salmon had those bin sizes and in the scatter plots the correlations were summed up. 
So, looking at the heat maps not stratified by length they look the same for all the NDR thresholds but the correlations are smaller on the gene level which could maybe be explained by the fact the we sum up the transcripts for the genes thus resulting in fewer data and a smaller correlation.The heat maps for the length stratification again look similar between the 4 NDR thresholds and show as expected highest correlation with their own sample between Salmon and Bambu as well as their replicates. This means that the individual samples between Bambu and Salmon do correlate well and are the most similar to their own sample in the other method and thus both methods seem to produce comparable results although the correlation is not as pronounced. This may be due to noise and outliers which even the spearman correlation isn't robust to. 

Lastly, looking at the scatter plot which again is almost the same for all 4 NDR thresholds there is a clear trend of a better summed up correlation towards longer lengths but a very low correlation for shorter transcript lengths (<=700 bp). This is due to the dropout in the PacBio data as it does not detect so short transcript lengths. The trend towards longer transcript lengths could be due to the assignment of PacBio reads to transcripts getting more accurate with longer transcripts. This means that the PacBio and Illumina data are getting more correlated with longer transcripts and are less comparable with respect to shorter transcripts. 
