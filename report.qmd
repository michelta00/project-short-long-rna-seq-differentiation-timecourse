...
To overcome these challenges, new technologies have been developed recently: for short-read sequencing Smart-seq3 has been introduced which is a sequencing approach that improves isoform detection by facilitating single-molecule reconstruction through the integration of reads with the same 5′ unique molecular identifier (UMI). The disadvantage of this technique is that it has a 5’ coverage bias which results in only partial reconstruction of most transcripts thereby not improving the isoform identification. Long-read technologies have been improved with respect to their raw base calling accuracies which were very error-prawn by using for example circularized consensus sequencing (Al’Khafaji *et al*, 2023). 
 
When performing single molecule, real-time (SMRT) Cell 8M chemistry on the current PacBio Sequel Ile platform, the base quality of the sequencing only shows minimal improvement after around 10 circular passes (Baid et al, 2023; Wenger et al, 2019).  However, with transcript lengths of around 200-5000 bp, circularized consensus sequencing of circularized cDNA using the ISO-seq protocol can lead to a much higher number of circular passes and thus does not exploit the full capability of the Ile Sequel platform (Al’Khafaji et al, 2023). A newly introduced method for long-read sequencing of transcript isoforms based on ISO-seq is multiplexed arrays isoform sequencing (MAS-ISO-seq), which was introduced by Al’Khafaji *et al* (2023). MAS-ISO-seq relies on the concatenation of cDNA fragments into longer sequence libraries with a narrow length distribution, the workflow is depicted in figure 1. In short, the cDNA fragments are distributed over multiple PCR’s, each of which appends specific barcode adapters containing deoxy-uracil (dU). Following, the amplified samples are pooled again and undergo dU digestion. Finally, the MAS-ISO-seq workflow creates concatenated cDNA fragments by barcode-directed adapter ligation of the original cDNA fragments. These concatenated fragments can then be used for PacBio sequencing (Al’Khafaji *et al*, 2023). 

![**Figure 1: MAS-ISO-seq workflow.** MAS-ISO-seq relies on the concatenation of cDNA fragments into longer sequence libraries to fully use the capacities of the PacBio Sequel Ile platform. Figure from Al’Khafaji *et al* (2023).](figures/intro_figure.png){width=60%}

The performance of the MAS-ISO-seq protocol was tested by Al’Khafaji *et al* (2023) using different sample sets, including the Lexogen SIRV-Set 4 (mixture of synthetic RNA sequences of known sequence, spike-in RNA variants (SIRV)) and a single cell sample set of tumor-infiltrating CD8+ T cells. In summary, the authors could show that RNA isoforms could be identified much more confidently with the MAS-ISO-seq protocol than with short-read methods when applied to both the SIRV sample set and the single cell sample set. Moreover, error robustness and >15-fold increase in throughput of the PacBio Sequel Ile platform could be shown (Al’Khafaji *et al*, 2023).
 
Here, we performed a similar analysis comparing two different count data sets, one derived from the MAS-ISO-seq protocol (long-read sequencing) and one derived from Illumina sequencing (short-read sequencing). The data sets contain samples from WTC-11 cells (induced pluripotent stem cells) that underwent a directed differentiation from pluripotent stem cells to hemogenic endothelial cells. More details can be found here: [directed-differentiation-hemogenic-endothelial-cells-from-human](https://app.jove.com/t/62391/directed-differentiation-hemogenic-endothelial-cells-from-human). For each day, including day 0, 1 – 3 samples were prepared. These samples were then sequenced using both the PacBio and Illumina platforms and were subsequently quantified with Bambu in the case of the PacBio sequence data and with Salmon in the case of Illumina sequence data. Bambu is an R package that performs transcript discovery and quantification for long-read RNAseq data (Chen *et al*, 2023), while Salmon does the same for short-read data (Patro *et al*, 2017). The quantification was performed using 4 different novel discovery rate (NDR) thresholds (NDR thresholds of 0.025, 0.05, 0.1 and 0.2 were applied here), which approximate the proportion of novel transcripts to control the balance between the sensitivity and the precision of the transcript discovery. The obtained data sets were finally compared in an exploratory data analysis (EDA), a differential gene expression (DGE) analysis and a differential transcript usage (DTU) analysis to explore the performance of the MAS-ISO-seq protocol compared to Illumina sequencing, which has been used and thus underwent optimization for much longer.
...
The exploratory comparison of PacBio+Bambu vs. Illumina+Salmon data was performed in RStudio (R version 4.3.2 (2023-10-31)). 
...
Lastly, the MDS plots were plotted for all 8 data frames (here we used the count data frames again) to compare the sample distribution between Illumina+Salmon and PacBio+Bambu. 
...
Before performing the differential expression analysis itself, some genes of interest were investigated. These genes are targeted by the directed differentiation treatment the cells underwent, and therefore might show changes in their expression over time. Here, the following genes were investigated:
...
The heat map for the pearson correlation looks unusual as not every sample is highest correlated with itself and the spearman heat maps look similar between the cutoffs but there is lower correlation on the gene level compared to the transcript level. Also the day 2 and day 3 bambu samples seem to be highly correlated also with the day 4 and day 5 salmon samples. 

Then, the correlation stratified by length was plotted for all 4 NDR cutoffs. For the purpose of no further information gain we will only show one pearson plot again and then representatively one spearman heat map (for one bin) for each NDR cutoff ( we chose 1000-2000 as its the most abundant transcript length) as well as all 4 scatter plots of the summed up correlation stratified by length. 
...
```
The pearson plots do not look like usual heat map as the day 2 - rep 1 - bambu is highly correlated with all samples of salmon and the rest of the correlations seem quite low. Also the scatter plot is widely spread and one can see no clear trend of the summed up correlation as it is jumping up and down. When observing the 0.025 heat map with spearman correlation one sees most of the samples being highly correlated with itself (or the replicates) and a bit clearer the highly correlated diagonal usually seen in heat maps. 
...
scatter_total <- scatter_total +
  ...
  plot_annotation(title = "Scatter plots of summed up correlations stratified by range of transcript lengths (0-10000)")
scatter_total

```
The rest of the heat maps for the other cutoffs (0.05, 0.1, 0.2) look very similar to the 0.025 heat map. Looking at the scatter plots of summed up correlations stratified by range of transcript lengths they also look almost the same between the different cutoffs. One can observe the clear trend that with a higher transcript length the correlation is also higher but for very small transcript lengths (<1000) the correlation is very low. 
...
To investigate the expression changes in the genes of interest (see above), their average expression at each time point was computed and the plotted.
...
## Salmon
tpm_salmon <- unname(unlist(avg_expr_salmon["ENSG00000105989.10", ]))
...
## Bambu
tpm_bambu <- unname(unlist(avg_expr_bambu["ENSG00000105989.10", ]))
...
title <- ggdraw() + draw_label("TPM of WNT2 (Wnt family member 2)", fontface='bold')
p <- plot_grid(plot_bambu, plot_salmon, ncol=2)
plot_grid(title, p, ncol=1, rel_heights=c(0.1, 1))
```

The first gene investigated is Wnt2 (Wnt family member 2). This gene was targeted by the treatment on days 0 and 1. Here, the addition of GSKi activated Wnt-signaling which in turn activated B-catenin providing transcriptional regulation. The plot above shows a general increase in Wnt2 expression over the time course, however, especially in the Salmon data, low (or no) expression is detected in the first days (days 0 - 2).
...
title <- ggdraw() + draw_label("TPM of CTNNB1 (catenin beta 1)", fontface='bold')
p <- plot_grid(plot_bambu, plot_salmon, ncol=2)
plot_grid(title, p, ncol=1, rel_heights=c(0.1, 1))
```

The next gene of interest is CTNNB1 (catenin beta 1), which is activated by Wnt-signalling. Here, a trend is visible showing an increase in expression in the first days, and then the expression reaches a plateau for the last days. Interestingly, even though CTNNB1 depends on the activation of Wnt-signalling, raising expression levels could be detected for CTNNB1 before Wnt2.

```{r echo=FALSE}
# SMAD1

## Salmon
tpm_salmon <- unname(unlist(avg_expr_salmon["ENSG00000170365.10", ]))
df_salmon <- data.frame(tpm = tpm_salmon, time = c(0, 1, 2, 3, 4, 5))
...
## Bambu
tpm_bambu <- unname(unlist(avg_expr_bambu["ENSG00000170365.10", ]))
df_bambu <- data.frame(tpm = tpm_bambu, time = c(0, 1, 2, 3, 4, 5))
...
title <- ggdraw() + draw_label("TPM of SMAD1 (SMAD family member 1)", fontface='bold')
p <- plot_grid(plot_bambu, plot_salmon, ncol=2)
plot_grid(title, p, ncol=1, rel_heights=c(0.1, 1))
```

On days 3 - 5, the treatment continues with the addition of BMP4 and VEGF. BMP4 induces the transition of mesodermal cells into endothelial lineages through usage of upstream Indian-Hedgehog signaling and downstream Smad signaling. Therefore, the gene SMAD1 (SMAD family member 1) was investigated next. The plot above shows slight increase in SMAD1 expression during the first days, and then more or less constant expression. These observations are surprising since BMP4 is only added starting from day 3.
...
title <- ggdraw() + draw_label("TPM of ETV2 (ETS variant transcription factor 2)", fontface='bold')
p <- plot_grid(plot_bambu, plot_salmon, ncol=2)
plot_grid(title, p, ncol=1, rel_heights=c(0.1, 1))
```

Lastly, the gene ETV2 (ETS variant transcription factor 2) was investigated. It is activated due to the addition of VEGF which is added starting from day 3 as described above. Here, almost no expression can be detected before day 3, followed by a major increase on day 3 and decreasing expression on days 4 and 5.

When comparing the expression levels from the Illumina+Salmon data and from the PacBio+Bambu data, the general trend of the expression levels seem to be similar.
