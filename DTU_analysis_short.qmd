...
The workflow was taken from [here](https://www.bioconductor.org/packages/release/workflows/vignettes/rnaseqDTU/inst/doc/rnaseqDTU.html) and tested on the given dataset. The workflow made use of 3 libraries vignettes: DRIMSeq, DEXSeq and stageR. DEXSeq assumes that the feature (transcript) counts follow a negative binomial (NB) distribution and considers them as relative proportions of the group (the gene) using an interaction term in a generalized linear model (GLM) [2, 3]. DRIMSeq assumes that feature proportions follow the Dirichlet distribution (an Dirichlet Multinomial model (MD) for each gene) where the total count for the gene is considered fixed [2, 4]. First, the a `dmDSdata` object aws created. The filtering was done using the function `dmFilter` from DRIMSeq package. *n* is defined as the minimal number of samples where the genes should be expressed. *n.small* is the minimal number of samples where the features should be expressed. Both are set to 3, as that is the number of replicates for day 0 and 5. Additional filters were also added: for a transcript to be retained in the dataset, it is required that (1) it has a count of at least 10 in at least *n.small* samples, (2) it has a relative abundance proportion of at least 0.1 in at least *n.small* samples, and (3) the total count of the corresponding gene is at least 10 in all *n* samples. The filtered DRIMSeq object was used for DEXSeq workflow, as the filtering was easier, and the analysis was sped up. `DEXSeqDataSet` was built from `d`, filtered DRIMseq object, sample information (`sample.data`), the design formula `~sample + exon + day:exon`, transcript IDs and gene IDs. DEXSeq analysis was performed using the following 3 functions: (1) estimation of size factors using `estimateSizeFactors`, (2) estimation of dispersion `estimateDispersions`, then (3) perform a likelihood ratio test for differential exon usage (in this case, transcripts) `testForDEU`. The results tables (log2 fold changes and p-values) were generated using the `DEXSeqResults `function and `perGeneQValue` function was used to compute a per-gene adjusted p-value. stageR package was used for post-processing of the calculated p-values and does stage-wise analysis of data, by screening first at the gene level for evidence of DTU (screening stage) and confirms which trancsripts within those significant genes show evidence of DTU (confirmation stage). `stageR` analysis was performed with *overall false discovery rate* (OFDR, alpha parameter) equal to 0.05. The data.frame `dex.padj` was generated and summarizes the adjusted p-values from the two-stage analysis. Only genes that passed the filter (screening) were included in the table. The transcripts (column `trancsript`) with values less than 0.05 (5 \% OFDR) passed the confirmation stage. A custom function `plotExpression` was made to plot the transcript expression of a gene as from [here](https://ycl6.gitbook.io/guide-to-rna-seq-analysis/differential-expression-analysis/differential-transcript-usage/dtu-using-dexseq). The inspiration for this protocol was taken from the following vignettes [2] and [5]. As for DGE, UpsetR plot was used to visualize the results along with Venn diagrams.
