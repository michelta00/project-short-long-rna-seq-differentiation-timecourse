...
In total, this adds up to 8 different data frames. Each of these data frames contains 11 samples, which are replicates from one of six different time points. Each time point has 1-3 replicates.
...
The differential gene expression analysis aims to identify genes that are differentially expressed over the time course. These results were computed for all 4 NDR thresholds of both technologies, and were then overlayed for each NDR threshold to compare the two technologies, so the long-read and short-read sequencing.
...
For Illumina+Salmon, out of the 24717 tested genes, 7343 were identified as differentially expressed. In the case of PacBio+Bambu, 7838 genes were identified as differentially expressed. Both methods identified roughly the same amount of genes as differentially expressed. 
...
plot_padj_0.025 <- ggplot(data = padj, mapping = aes(x = -log10(Salmon), y = -log10(Bambu))) +
  geom_point() + 
  geom_smooth() +
  theme_minimal() +
...
plot_logfc_0.025 <- ggplot(data = logfc, mapping = aes(x = Salmon, y = Bambu)) +
  geom_point() + 
  geom_smooth() +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5)) +
...
plot_comparison_0.025 <- plot_padj_0.025 + plot_logfc_0.025
plot_comparison_0.025
```

The comparison of the p-values after adjustment shows a similar trend in both technologies, but also some differences. The same is observable for the logFC values. Additionally, it is visible that many genes that show a logFC value of zero in the analysis of one technology show a (high) logFC value in the other one, which agrees with the observation made in the UpSet plot, that some genes are called differentially expressed by either of the two technologies, but not by both.
...
plot_dge_0.025 <- plot_dge_0.025_salmon + theme(legend.position = "none") + plot_dge_0.025_bambu
plot_dge_0.025
```

The analysis identified 2377 down-regulated and 4057 up-regulated genes for Illumian+Salmon and 2014 down-regulated and 3144 up-regulated genes for PacBio+Bambu. Again, 24717 genes were tested and both technologies show similar results.

#### NDR 0.05

The next NDR threshold was 0.05. The same analysis steps that were performed for the NDR threshold of 0.025 were performed here.
...
For Illumina+Salmon, out of the 24811 tested genes, 7367 were identified as differentially expressed. In the case of PacBio+Bambu, 7884 genes were identified as differentially expressed. Both methods identified roughly the same amount of genes as differentially expressed. 
...
The UpSet plot shows similar results compared to the NDR threshold of 0.025. Again, the majority of genes were detected by both methods, however, some were detected by either of the two methods.
...
plot_padj_0.05 <- ggplot(data = padj, mapping = aes(x = -log10(Salmon), y = -log10(Bambu))) +
  geom_point() + 
  geom_smooth() +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5)) +
...
plot_logfc_0.05 <- ggplot(data = logfc, mapping = aes(x = Salmon, y = Bambu)) +
  geom_point() + 
  geom_smooth() +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5)) +
...
plot_comparison_0.05 <- plot_padj_0.05 + plot_logfc_0.05
plot_comparison_0.05
```

The comparison of adjusted p-values and logFC values shows the same results as seen when performing the comparison on the results of the 0.025 NDR threshold. 
...
plot_dge_0.05 <- plot_dge_0.05_salmon + plot_dge_0.05_bambu
plot_dge_0.05
```

The differential expression analysis of day 5 and day 0 identified 2388 down-regulated and 4071 up-regulated genes for Illumian+Salmon and 2031 down-regulated and 3172 up-regulated genes for PacBio+Bambu. Again, 24811 genes were tested and both technologies show similar results.

#### NDR 0.1

The next NDR threshold was 0.1. The same analysis steps that were performed for the other NDR thresholds were performed here as well.
...
When overlaying the results of the two technologies, it is again visible that most genes were detected by both methods, while some were deteced exclusively by one of them.
...
plot_padj_0.1 <- ggplot(data = padj, mapping = aes(x = -log10(Salmon), y = -log10(Bambu))) +
  geom_point() + 
  geom_smooth() +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5)) +
...
plot_logfc_0.1 <- ggplot(data = logfc, mapping = aes(x = Salmon, y = Bambu)) +
  geom_point() + 
  geom_smooth() +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5)) +
...
plot_comparison_0.1 <- plot_padj_0.1 + plot_logfc_0.1
plot_comparison_0.1
```

The comparison of adjusted p-values and logFC values shows the same trends as seen for the other NDR thresholds.
...
plot_dge_0.1 <- plot_dge_0.1_salmon + plot_dge_0.1_bambu
plot_dge_0.1
```
...
The last NDR threshold was 0.2. Again, the same steps are performed.
...
The whole time course differential expression analysis identified 7723 deferentially expressed genes for Illumina+Salmon and 8345 for PacBio+Bambu. For the NDR threshold of 0.2, 26012 genes were tested.
...
The overlay of the results and the logFC/p-value comparsion shows similar results compared to the other NDR thresholds.
